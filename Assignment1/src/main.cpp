...
//Initialize radius
  ...
  //Initialize sphere center
  centerX = viewport.w / 2.0f;
  centerY = viewport.h / 2.0f;
  centerZ = min(viewport.w, viewport.h) / 2.0f;
  ...
  for (int i = 0; i < argc; i++) {
    ...
    if(arg == "-ka"){
      ...
      k_ward = glm::vec3(atof(argv[i+1]), atof(argv[i+2]), atof(argv[i+3]));

      i+=3;
    } else if(arg == "-kwv"){
      ward_vec = glm::normalize(glm::vec3(atof(argv[i+1]), atof(argv[i+2]), atof(argv[i+3])));

      i+=3;
    } else if(arg == "-kwc"){
      k_ward_x = atof(argv[i+1]);
      k_ward_y = atof(argv[i+2]);
      ...
      ::new(&point_lights[num_pl]) PointLight(atof(argv[i+1]), atof(argv[i+2]), atof(argv[i+3]), centerX, centerY, centerZ, unit_radius, atof(argv[i+4]), atof(argv[i+5]), atof(argv[i+6]));
